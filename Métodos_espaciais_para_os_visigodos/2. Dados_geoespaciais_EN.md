...
Although our list of places is rather long, rather quickly we encounter places which we cannot locate easily. The first case is Narbona. How do we locate Narbona and enter it's coordinates?

As you might notice there are some places that have a Pleiades code. This code refers to the [Pleiades](https://pleiades.stoa.org/) dataset (Yes you can also download the whole set under the tab [Downloads](https://pleiades.stoa.org/downloads)). Here you can search most ancient places. We can search by ancient or modern name. Note that spelling is important, but partial names also gives results. So best to write the bare minimum in order to find your result.
