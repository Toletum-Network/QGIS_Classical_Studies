...
Now we have a basic map of Pacensis and we can upload the shapefile with the inscriptions from the conventus Pacensis. The inscriptions are all as single dots on the map. This way we have no way to work with them. Take a look at the attribute table to see 

### 13.4.1 Filtering Data
We will query and filter this data to create different kinds of maps. For this we will go one step deeper in to the Rule-based symbology.
...
When we have written our expression correctly we will see to the bottom left "Preview: 0". If we have a mistake the Preview states "Expression is invalid". Check your expression for mistakes. Note that these could also be in the quotation marks " or ' (not `). 
