...
Now we have a basic map of Pacensis and we can upload the shapefile with the inscriptions from the conventus Pacensis. 
...
Now we are going to add another filter. We want to only see the funerary inscriptions on marble.
The expression needed:
 "Text Type" LIKE  '%Funerary%' 
 AND 
 "Material" LIKE '%Marble%'
 
 **``! Note``** 
 It is not needed to write each expression on one line. Hoerver, for readability of larger expressions it is good practice.
