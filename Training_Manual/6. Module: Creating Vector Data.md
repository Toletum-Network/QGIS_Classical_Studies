...
1. Open QGIS and open the _**GIS Course Module 6**_ project.
   ...
   You will notice there is already a layer available in this view. These are Web Map Service (WMS) layers running from the server of the Instituto Geográfica Nacional (IGN). This makes it possible for us to view the map of Spain without having to save a multitude of large Raster files on our local discs. However, the downside is that it needs an internet connection and takes some time to load the map tiles. We will use this WMS later in this module. First we will make an excursion to another site on the Iberian Peninsula: Italica.
...
Next there is a collection of fields grouped under **``New Field``**. By default, a new layer has only one attribute, the _id_ field (which you should see in the **``Fields list``**) below. However, in order for the data you create to be useful, you actually need to say something about the features you’ll be creating in this new layer. For our current purposes, it will be enough to add one field called _Type_ that will hold _Text data_ and will be limited to text length of _80_ characters.
...
After this short excursion to the site of Italica we will return to the Almanzora basin.
...
1. Create a new ESRI Shapefile line dataset called _routes.shp_, with the same attributes as the roads (use the approach above to guide you.) Using the same attributes as the layer it could belong to makes it easier to join the layers in a later stage.
