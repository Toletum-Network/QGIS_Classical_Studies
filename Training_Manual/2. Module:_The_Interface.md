...
At the moment the Browser Panel might be missing from your interface, this can be solved easily via **`View ‣ Panels`** menu. After ticking the box next to Browser you should have the Browser Panel added. You can do this for other panels and toolbars.
...
This is where the map itself is displayed and where layers are loaded. In the map canvas you can interact with the visible layers: zoom in/out, move the map, select features and many other operations that we will see repeatedly in the next sections.
...
* Click on the ![zoomFullExtent](https://github.com/Toletum-Network/AutumnSchool_2020/blob/master/Icons/mActionZoomFullExtent.png)**`Zoom Full Extent`** button.
