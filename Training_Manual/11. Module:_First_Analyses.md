...
Open **``Vector ‣ Geometry Tools ‣ Voronoi Polygons``**. Choose the **L11_AWMC_rCities.shp**, an *Buffer Region* of 10 (that helps to cover the whole landmass even at borders where no city is located. You can try without any buffer to see the difference)  just click **``Run``**. Your result should look like this:
