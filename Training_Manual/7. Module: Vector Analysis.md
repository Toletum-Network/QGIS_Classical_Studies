...
7.3.1. basic Follow Along: The Tools and the Data
...
7.3.2. basic Calculate the shortest path (point to point)
...
Use Shortest for Path type to calculate

Use these two points as starting and ending points for the analysis:

../../../_images/start_end_point.png
Click on the … button next to Start point (x, y) and choose the location tagged with Starting Point in the picture. The coordinates of the clicked point are added.
...
Open the attribute table of the output layer. It contains three fields, representing the coordinates of the start and end points and the cost.
...
7.3.3. moderate Try Yourself Fastest path
...
7.3.5. hard Shortest map with speed limit
...
Click on the … button next to the Start point (x, y) and choose the start point.

Do the same thing for End point (x, y)

Open the Advanced parameters menu

Choose the speed field as the Speed Field parameter. With this option the algorithm will take into account the speed limits for each road.
...
7.3.6. moderate Service area (from layer)
...
Enter 250 for the Travel cost parameter
...
7.4.1. basic Follow Along: Create a Test Dataset
...
7.4.3. basic Follow Along: Compute statistics on distances between points
...
7.4.6. basic Follow Along: Image Histograms
