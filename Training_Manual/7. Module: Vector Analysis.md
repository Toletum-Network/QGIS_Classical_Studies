...
For the next three modules, we’ll endeavor to answer a _research question_ using GIS functions. For example, you are interested in the possible locations for villae with the following criteria (note these are not based on historical requierements!):
...
Let’s start off the process by deciding on a problem to solve. For example, you are looking for possible locations of villae sites near Tagili:
...
These data are available in the dataset you have been using throughout this manual also can be used for this lesson.

**``! Note``**
If you want to download data from another area, jump to the [Introduction Chapter](https://github.com/Toletum-Network/AutumnSchool_2020/blob/master/Training_Manual/1.%20Course%20Introduction.md) to read how to do it. Note that it will take time to select and create the needed data.
