...
1. Add another vector layer to your map which has the data for Spain only. You will find it in the Module 7 *__exercise_data**_ as _**L07_Pleiades_IP_Places_(WGS84)__*.
...
1. Right-click on the _**L07_Pleiades_IP_Places_(WGS84)**_ layer in the **``Layers``** panel
...
4. Navigate to _**exercise_data/**_ and specify the name of the new layer as _**L07_Pleiades_IP_Places_(UTM 30N)**_.
...
1. Make sure that only the _routes_ and _**BTN100_rivers**_ layers are visible (to simplify the map while you’re working)
...
2. Select _CLC_LanduseAlmanzora_ in **``Extract features from``**. Check **``intersect``** in **``Where the features (geometric predicate)``**, select the buffer intersection layer in **``By comparing to the features from``**. Save and name the layer _**LanduseAlmanzora_extracted**_.
