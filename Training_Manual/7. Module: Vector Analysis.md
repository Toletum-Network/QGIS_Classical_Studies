...
1. They are within roughly 1h walking (5 km) from Tagili.
2. They must be on agricultural land.
3. Closer than 1.5 km to the main road.
...
1. Right-click on the _L07_Peiades_IP_Places_(WGS84)_ layer in the **``Layers``** panel
...
4. Navigate to _exercise_data/_ and specify the name of the new layer as _L07_Peiades_IP_Places_(UTM 30N)_.
...
2. In the Module 7 exercise data, you will find the datasets we will use in this chapter. You can also use the data you have been suing throughout the course:
   ...
   3. routes for the main road through the basin
   4. _L07_BTN100_rivers_ for the rivers
...
Before proceeding we will filter the _CLC_LanduseAlmanzora, in order to have only some specific landuse types to work with.
...
**``! Note``** When trying to use the expression above with the CODE_90 field it does not work. Eventhough both field seem to hold the same numerical values in the attribute table, the outcome is different. Check the **``Layer Properties ‣ Manage Fields``**  to see the difference between the Field type of the fields _Code_ and _CODE_90_. As the _CODE_90_ field type is _String_ the values in the field are treated as text rather than numbers. Therefore the expression based on numerical values does not work for _CODE_90_. 
...
### 7.2.5. ![basic](https://github.com/Toletum-Network/AutumnSchool_2020/blob/master/Icons/basic.png) Follow Along: Analyzing the Problem: ~Distances From Rivers and Roads~
...
### 7.2.6. ![basic](https://github.com/Toletum-Network/AutumnSchool_2020/blob/master/Icons/basic.png) Try Yourself Distance from cities
